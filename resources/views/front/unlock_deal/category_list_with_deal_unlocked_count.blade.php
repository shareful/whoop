...
to boost your savings 
    ...
    <script type="text/javascript">
        ...
        url = API_BASE_URL+"category/list/with/deals_unlocked/count";
        ...
        $.ajax({
            ...
            if(length>0) {
                ...
                for (var i = 0; i<length; i++) {
                    $html +="<div class='col-4 m_t_10 m_b_10 rm-p0 text-center'><div class='pull-right number_sec'><span class='number_font'>"+record[i].unlocked_deals_count+"</span></div><span><a href='"+URL+"/user/category/"+record[i].id+"/deals_unlocked/list'><img src='"+record[i].image+"'></a></span><p class='font'>"+record[i].name+"</p></div>";
                }
                $html += "</div></div>";
